...
export async function GET(req: NextRequest, { params }: RouteContext) {
  ...
  const { data: course, error } = await getSupabase().from("courses").select("*").eq("id", id).single();
...
export async function PUT(req: NextRequest, { params }: RouteContext) {
  ...
  const { data: course } = await getSupabase().from("courses").select("*").eq("id", id).single();
  ...
  const { data, error } = await getSupabase().from("courses").update(updates).eq("id", id).select().single();
